...
public class HW6TestExt {
	...
	boolean isTrie, isHybrid;
	
	private StringTries<Integer> createST() {
		StringTries<Integer> st;
		if (isHybrid) st = new HybridT<Integer>();
		else if (isTrie) st = new Trie<Integer>();
	...
	private void testMethod(int methodID) throws Exception {
		try {
			isHybrid = (methodID > 6);
			isTrie = (methodID > 3);
			if (isHybrid) methodID -= 6;
			else if (isTrie) methodID -= 3;
	...
	@Test
	public void testHybridTrieSize() throws Exception {
		test = "* Testing size (Hybrid)                   *\n";
		testMethod(7);
	}
	
	@Test
	public void testHybridTrieRank() throws Exception {
		test = "* Testing rank (Hybrid)                   *\n";
		testMethod(8);
	}
	
	@Test
	public void testHybridTrieContains() throws Exception {
		test = "* Testing containsKeyEndingIn (Hybrid)    *\n";
		testMethod(9);
	}
}