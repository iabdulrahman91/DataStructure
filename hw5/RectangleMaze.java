...
public class RectangleMaze extends GenericMaze {
	private LinkedList<IntPair> exits;
	private LinkedList<IntPair> weapons;
	private LinkedList<IntPair> monsters;
	
	public RectangleMaze(IntPair size, Random rand, int numExits, int numMonsters, int numWeapons) {
		super(size);
		if (numExits + numMonsters + numWeapons + 1 > size.x * size.y) {
